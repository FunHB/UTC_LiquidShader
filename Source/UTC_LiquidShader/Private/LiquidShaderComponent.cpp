...
void ULiquidShaderComponent::BeginPlay()
{
    ...
    InitialLiquidOffset = GetLiquidOffset();
    PreviousLiquidFillAmount = LiquidFillAmount;
...
void ULiquidShaderComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
    ...
    if (WavesRotationStrength != 0 || WavesRotationVectorStrength != TargetVector || WavesOffsetStrength != 0 || WavesOffsetVectorStrength != TargetVector || PreviousLiquidFillAmount != LiquidFillAmount)
    {
    ...
    /** Change Color */
    LiquidShaderDynamicMI->SetVectorParameterValue(TEXT("Liquid - Diffuse"), LiquidColor);
    LiquidShaderDynamicMI->SetVectorParameterValue(TEXT("Liquid - MFP Color"), LiquidColor);
}
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            /** Set Liquid Offset */
                            if ((FMath::Clamp(LiquidFillAmount, 0.f, 1.f) * InitialLiquidVolume) <= VolumeAmount)
                            {
...
void ULiquidShaderComponent::EnqueueCS()
{
    ...
    FQuat Rotation = SMComponent->GetComponentRotation().Quaternion();

    /** Execute Cap Shader */
    auto IntersectionCS = new (FUTC_LiquidShaderCSManager);

    IntersectionCS->PerformIntersection(VertexBuffer, IndexBuffer, AdaptedLiquidOffset, BoundRange, SectionRange, Rotation, CapRTInstance);
